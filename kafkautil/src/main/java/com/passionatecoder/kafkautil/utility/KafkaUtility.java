...
public class KafkaUtility {
		...
		try (final AdminClient adminClient = getAdminClient()) {
			 ...
			 if(null != topics) {
				...
				for(String topic : topicNames) {
					...
					LOGGER.info("Listing name : " +listing.name() + " isInternal : "+listing.isInternal());
				}
			 }
	...
	public static void main(String[] args) throws JsonGenerationException, JsonMappingException, IOException {
		System.out.println(new ObjectMapper().writeValueAsString(getAdminConfig()));
		System.out.println("**************" +PropertiesUtil.getTest1Topic());
		getAllTopics();
		printTopicDescription();
		//createTopic(PropertiesUtil.getDQTopic(),5,1);
...
//		createTopic("audit-topic2",5,1);
		...
		//createTopic(PropertiesUtil.getPssMessageTopic(),1,1);
		//deleteTopic(PropertiesUtil.getPssMessageTopic());
		
		//createTopic("distributed-video1",1,1);
	}
